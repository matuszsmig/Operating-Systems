...
void count(struct PointerArray *pointerArray, char *file){
    char * mkFile = calloc(255, sizeof(char));
    char * rmFile = calloc(255, sizeof(char));
    char * countValues = calloc(255, sizeof(char));
    ...
    strncpy((char *) pointerArray->pointerArray[pointerArray->currentPostion].file,
            countValues, itr - 2 - len);
...
char *valueOnIndex(struct PointerArray *pointerArray, int index){
    if (pointerArray->pointerArray[index].isActive == 1){
